...
class ConvSelfAttention(nn.Module):
    ...
    def __init__(self, n_channels):
        ...
        self.query, self.key, self.value = [
            self._conv(n_channels, c) for c in (n_channels // 8, n_channels // 8, n_channels)
        ]
...
class ConvSelfAttentionMobileNet(nn.Module):
    def __init__(self, model_setting, n_classes, last_channel=128, in_channels=1):

        super().__init__()
        self._config = model_setting
        ...
        features = [ConvBNReLU(self.in_channels, input_channel, stride=1, norm_layer=norm_layer)]
        for t, c, n, s in model_setting:

...
def get_augm_func(time_mask_param=80, freq_mask_param=16, crop_augm_max_cut_size=0):
    ...
    if crop_augm_max_cut_size != 0:
        ...
        def crop_f(tens):
            ...
            random_crop = transforms.RandomCrop(np.array(tens.shape)[1:] - np.array([0, crop_delta]))
...
class MelEmotionsDataset(Dataset):
    def __init__(self, df, *_, augm_transform=None, get_weights_func=None, base_path=None, **__):
        super().__init__()
        df = df.copy()
...
class LengthWeightedSampler(Sampler[int]):
    def __init__(
        ...
        for i, v in enumerate(all_lengths):
            if v > temp_length:
                if v != temp_length + self.length_step:
                    for j in np.arange(temp_length + self.length_step, v, self.length_step):
        ...
        length_to_index_mapping[np.round(np.max(all_lengths) + self.length_step, decimals)] = len(df)

        self.length_to_index_mapping = length_to_index_mapping

        # starts with MIN_LENGTH
        self.lengths, self.lengths_count = np.unique(
            all_lengths[length_to_index_mapping[self.min_length] : length_to_index_mapping[max_plus_delta]],
            return_counts=True,
        )

        self.key_length_sampler = Categorical(probs=torch.from_numpy(self.lengths_count))
    ...
    def __iter__(self):
        ...
        while N < self.num_samples:
            ...
            batch_min_length = np.round(max(self.min_length, key_length - self.length_delta), self.decimals)
            batch_max_length = np.round(min(self.max_length, key_length + self.length_delta), self.decimals)
            batch_max_length_plus_delta = np.round(batch_max_length + self.length_step, self.decimals)

            sub_df = self.df.iloc[
                self.length_to_index_mapping[batch_min_length] : self.length_to_index_mapping[
                    batch_max_length_plus_delta
                ]
            ][["sampling_weights"]]

            sampling_weights = torch.from_numpy(sub_df.sampling_weights.values.astype(float))
            sub_iloc_indexes = torch.multinomial(sampling_weights, self.batch_size, True).tolist()
